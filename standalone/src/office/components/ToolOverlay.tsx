...
import type { SubagentCharacter } from '../../hooks/useWebSocket'
...
export function ToolOverlay({
  ...
  agents,
  agentTools,
  agentResponses: _agentResponses,
  agentLastMessages,
  subagentCharacters,
  containerRef,
  zoom,
  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        // Get the last assistant message for tooltip (only for permission state)
        const lastMsg = hasPermission && !isSub ? agentLastMessages[id] : undefined
        const tooltipText = lastMsg
          ? (lastMsg.length > TOOLTIP_MAX_LENGTH ? lastMsg.slice(0, TOOLTIP_MAX_LENGTH) + '...' : lastMsg)
          : undefined
