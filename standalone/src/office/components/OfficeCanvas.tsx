...
export function OfficeCanvas({ officeState, onClick, onFurnitureClick, isEditMode, editorState, onEditorTileAction, onEditorEraseAction, onEditorSelectionChange, onDeleteSelected, onRotateSelected, onDragMove, editorTick: _editorTick, zoom, onZoomChange, panRef }: OfficeCanvasProps) {
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (canvas) {
        ...
        if (cursor === 'default' && officeState.selectedAgentId !== null && tile) {
    ...
    [officeState, screenToWorld, screenToTile, isEditMode, editorState, onEditorTileAction, onEditorEraseAction, onFurnitureClick, panRef, hitTestDeleteButton, hitTestRotateButton, clampPan],
  )
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // No agent hit — check furniture click (TV etc.)
      if (onFurnitureClick) {
        const tile = screenToTile(e.clientX, e.clientY)
        if (tile) {
          const layout = officeState.getLayout()
          for (const f of layout.furniture) {
            ...
            if (!entry) continue
            if (tile.col >= f.col && tile.col < f.col + entry.footprintW && tile.row >= f.row && tile.row < f.row + entry.footprintH) {
              // Check if it's a TV type
              if (f.type.toUpperCase().includes('TV')) {
                onFurnitureClick(f.type, f.uid)
                return
              }
            }
          }
