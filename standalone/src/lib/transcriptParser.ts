...
export function processTranscriptLine(
  ...
  try {
    ...
    if (record.type === 'assistant' && Array.isArray(record.message?.content)) {
      ...
      // Capture the latest assistant text for hover tooltip
      ...
      if (textParts.length > 0) {
        agent.lastAssistantText = textParts.join('\n')
      }
      ...
      if (hasToolUse) {
        ...
        if (hasNonExemptTool) {
          startPermissionTimer(agentId, agents, permissionTimers, PERMISSION_EXEMPT_TOOLS)
        }
