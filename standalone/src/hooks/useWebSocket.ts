...
import { sendMessage, setOnMessage, connectWebSocket } from '../lib/messageApi'
import { playDoneSound, setSoundEnabled } from '../notificationSound'

// Shared server CWD, set by settingsLoaded message
let _agentCwd = ''
export function getAgentCwd(): string { return _agentCwd }
...
export interface ExtensionMessageState {
  ...
  agentLastMessages: Record<number, string>
...
export function useWebSocket(
  ...
  const [agentLastMessages, setAgentLastMessages] = useState<Record<number, string>>({})
  ...
  useEffect(() => {
    ...
    const handler = (msg: Record<string, unknown>) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        const id = msg.id as number
        const lastMessage = msg.lastMessage as string | undefined
        setAgentTools((prev) => {
          const list = prev[id]
          if (!list) return prev
          return {
        ...
        if (lastMessage) {
          setAgentLastMessages((prev) => ({ ...prev, [id]: lastMessage }))
        }
        ...
        if (msg.agentCwd) {
          _agentCwd = msg.agentCwd as string
        }
      ...
      } else if (msg.type === 'transcriptData') {
        const id = msg.id as number
        const entries = msg.entries as TranscriptEntry[]
        const jsonlFile = msg.jsonlFile as string
        const hasPty = (msg.hasPty as boolean) || false
        const ptyBuffer = (msg.ptyBuffer as string) || undefined
        const cwd = (msg.cwd as string) || undefined
        setTranscriptData({ agentId: id, entries, jsonlFile, hasPty, ptyBuffer, cwd })
      }
    }
    setOnMessage(handler)
    connectWebSocket()
    // No need to send webviewReady — server fires it automatically on WS connect
    return () => setOnMessage(null)
  }, [getOfficeState])

  return { agents, selectedAgent, agentTools, agentStatuses, agentResponses, agentLastMessages, subagentTools, subagentCharacters, layoutReady, loadedAssets, transcriptData, setTranscriptData }
}
