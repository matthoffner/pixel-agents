...
export interface EditorActions {
  ...
  handleOpenClaude: (options?: { cwd?: string; continueSession?: boolean }) => void
...
export function useEditorActions(
  ...
  const saveLayout = useCallback((layout: OfficeLayout) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      sendMessage({ type: 'saveLayout', layout })
  ...
  const handleOpenClaude = useCallback((options?: { cwd?: string; continueSession?: boolean }) => {
    sendMessage({ type: 'openClaude', ...options })
  }, [])
